...
docs/doodle-art-animation/          not shipped with the plugin
  ...
  v0.14-state.md … v0.16.5-state.md what each release changed, how it was checked, its known limits
...
- Sound tests, also run by CI: `python3 tests/doodle-art-animation/effects_test.py --node-modules <playwright>/node_modules` renders
  `fixtures/story_sounds.js` (one of each sound added in v0.16.5) and checks the header's writing sound follows the paper.
