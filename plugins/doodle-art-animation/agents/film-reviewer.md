...
3. **Measure the MP4**, if there is one:
   ...
   - `python3 audio_check.py <film>.mp4 --starts <transition times from text_check> --profile`. It exits 1 only on a hard failure (no audio, mono, clipping, audio and video lengths more than 0.2 s apart). If the story was made with `defineStory({ silent: true })`, run `python3 audio_check.py <film>.mp4 --silent` instead and skip the sound rubric below: a silent track has two identical channels, and the normal check would fail it as mono. If the film is narrated, add `--narrated`: the level targets are then the narration ones (−16 LUFS integrated, true peak at or under −1 dBTP), and the mean level is only reported.
...
- **Paper.** Read the paper set from `defineStory({ paper })` (none means `notebook`) and any `plate.paper`. On the contact sheet, every subject has to read on its paper: a fill that sinks into the ground (a dark fill on blueprint or fluorescence, a pale one on whiteprint) is a fail, with the fix (an outline in the paper's ink, a lighter fill, `PAL.bpLine` linework). Blueprint, whiteprint and fluorescence treat the whole scene, so colour alone cannot carry meaning there: two subjects told apart only by hue fail on the tinted blueprint papers. One film should use one set; a plate on another set or paper needs a reason the script gives. Check the set fits the topic (`references/style.md`, "Choosing the paper set").
...
- **Sound.** From `audio_check`: mean level -21 to -18 dB, peak about -3 dB (the example measures -20.2 and -4.7), no clipping, real stereo, no silent stretch, audio as long as the video. From `--starts`: each transition should have a sound onset within about 0.25 s, or a swell (the level climbing 6 dB or more within half a second, which `audio_check` passes: the engine's own seams dip and then rise). A warning means neither: `bleed`, `fade` and `hatch` can still warn with their slow swells, which is normal; a warning on a `cut`, `pan`, `lensIn` or `zoom` is not. From the story's `cues` and `cue_check`: different kinds of event get different sounds (`references/sound.md`) — a contact, a crunch, a liquid, a camera move and a reveal should not share one effect — no effect dominates or repeats identically, the engine's own header scratch is enough under typed text (don't add a `scratch` cue under every line: that is how a film ends up one-third pen noise), and no cue fires where nothing happens on screen. Compare cue times with the beat times you listed in step 1.
- **Screen and voice** (narrated films only: `brief.md` says narration is on, the plates have `vo`, or `vo/voice.json` exists; otherwise skip this). What the narrator says and when is in `<film>.srt`, the captions a narrated render writes next to the MP4, or in `window.__voice()` (each clip's plate, start, sentences and marks on the film clock). Compare them with the beats you listed in step 1:
  - on-screen text agrees with what is being said at that moment: a stat shows while or just after the narrator names it, not seconds before, and its number matches the spoken one ("≈ 430 billion" beside "about four hundred and thirty billion");
