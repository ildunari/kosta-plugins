...
- **Keep loud cues off important words.** A `chime`, `crunch`, `thump` or `pop` on the word that carries the meaning (the number, the new term) masks it. Put dense or loud cues in the narration's pauses. A cue time can be a mark name, like `['count+0.2', 'chime']`, so it lands just after the word it belongs to.
