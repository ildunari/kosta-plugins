...
description: Make hand-inked, notebook-style explainer films drawn entirely in canvas code from any topic, dataset or source, with lens zooms, ink wipes, paper burns, morphs, synthesized sound and optional voice-over narration, rendered to MP4.
---
...
Every pixel and every sound is computed by JavaScript in one HTML file with one `<canvas>`. `renderFrame(f)` is a pure function of the frame number, and a Playwright script captures the frames and muxes them with ffmpeg. There are no images, video clips, audio files or animation libraries. The one exception is a narrated film's own voice-over: `toolkit/voice.mjs` records it with a text-to-speech service and `build.py` embeds the clips in the HTML, so the film is still one file (`references/voice.md`).
...
| Gate 1 · Plan review | `script.md` on disk | a reviewed plan the user has seen | during the wait: folder, `helpers.js`, sound plan, and for a narrated film an audition only when two or three narrator presets fit equally |
| 2b · Voice (narrated films only) | a reviewed `script.md` with its narration | `vo/voice.json` locked, plate durations updated, an animatic | — |
| 3 · Sound | a reviewed `script.md` | `cues.md` | runs beside phase 4 |
| 4 · Art | a reviewed `script.md`, `helpers.js`, and for a narrated film the locked `vo/voice.json` | one `plate_<n>_<slug>.js` per scene | one agent per scene |
...
Five commands run the phases, and they are the **user's** to type — a model cannot invoke them: `/doodle-art-animation:doodle-plan` runs 0 to Gate 1, `:doodle-voice` runs 2b for a narrated film, `:doodle-build` runs 3 to 5, `:doodle-qa` runs Gate 2, `:doodle-render` runs 6. Suggest them when the user would rather drive phase by phase; otherwise follow the same phases yourself, in this order, which is what the rest of this file describes.

## Workflow

0. **Ask before you start.** Offer the user a short question round (3–7 questions, with options and a recommendation) and skip it if they said to just make it; see `references/intake.md`. Write the answers into `brief.md` and carry them into every phase below. One of the questions is narration: none (the default unless the request asks for narration or a voice-over), or a style, provider and voice (`references/voice.md`).
...
2b. **Record the narration** (narrated films only; `/doodle-art-animation:doodle-voice` runs it). After Gate 1 and before any art: `node voice.mjs generate`, `check`, retakes for what fails, then `lock`, which writes `vo/voice.json` and lengthens the plates the voice outgrew in `script.md`. Then build the animatic (`python3 build.py story.js film.html --animatic` on a story of placeholder plates, or render any build with `--animatic`) to judge the pacing with the real voice. The picture is timed to the voice, never the other way round: drawing starts only once the voice is locked. The voice is the narrator preset in `brief.md` (`references/voice-presets.md`); in unattended runs the thread picks it by that file's rules and carries on (`references/voice.md`).
...
7. **Deliver** the MP4 and the HTML, and for a narrated film the `film.srt` captions the render wrote. The HTML is also a player: space plays and pauses, the arrow keys move 2 s, `[` and `]` jump between plates, `c` shows captions, and there is a scrubber.
...
- **Narration: words, then voice, then picture.** A narrated film's words are written with the script, recorded and locked before anything is drawn, and each plate is timed to its clip (`vo: 'P2'`), with beats landing on marked words (`mark('count')`). The narrator never reads out a sentence that is also on screen; short labels and numbers are fine. The mix targets −16 LUFS with the music 15–20 dB under the voice (`references/voice.md`, `references/sound.md`, "Narration").
...
- **Narration** (narrated films). `node voice.mjs check` flags no clip; `audio_check.py film.mp4 --narrated --stems qa` passes after `node render.mjs film.html --stems --dir qa`; `film.srt` sits next to the MP4; each stat or label shows while or just after the narrator names it, never seconds before, and agrees with what is said.
