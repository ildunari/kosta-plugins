...
| `toolkit/story_seams.js` | "Pencil to Ladybug" (27.5 s): a short example of designed seams (`through`, an auto-direction `pan`, a match `cut`, a `page` turn). Read it with "Designing the seams" in `references/writing.md`. |
...
- **Seams** (`--seams`): the exit and entry objects line up in the overlay, motion keeps its direction across the cut, and nothing pops in or vanishes at the join.
- **Collisions.**
