...
| `references/intake.md` | The short question round with the user before a film starts, and the timed plan card at Gate 1. |
...
| 1 · Substance | `brief.md` | `facts.md`, a source per number | topic research · the user's data · numbers and citations |
...
The commands run the phases for you: `/doodle-art-animation:doodle-plan` runs 0 to Gate 1, `:doodle-build` runs 3 to 5, `:doodle-qa` runs Gate 2, `:doodle-render` runs 6. Use them in that order, or follow the same phases by hand.
...
1. **Get the substance first.** Research the topic, or read the user's data or source. Every number on screen needs a source; write the sources down now, because they go on the end card. When the topic is broad, fan this out into parallel lanes — topic research, the user's own data or sources, and checking the numbers and citations — and merge what comes back into `facts.md`. A small topic, or one where the user supplied everything, is faster done in one pass.
2. **Pick the hero subject and write the scene script.** The hero is the one subject the viewer follows through the whole story, at any size: a person, an animal, a vehicle, a letter, a coin, a data packet, a molecule. Give it an ID tag like `H₂O·01`, `NP·01`, `FOX·01`, `PKT·01` (see `references/writing.md`). Then write the scene script — one row per plate, in the table format from `references/writing.md`, with camera, transition and exit columns — and **design every seam** (exit, entry, link) in the seam list below the table, letting the link pick the transition. Plan how the motion flows: inside each scene, animations overlap and hand off to each other instead of stopping and starting, and each seam carries motion from one scene into the next (`references/animation-principles.md`). Save it as `script.md` in the film folder.
Gate 1. **Review the plan, then tell the user.** Run the `script-reviewer` agent on the saved `script.md` — never before that file exists — and apply the edits it calls must-fix. Then post a short summary of the plan as a timed card (`references/intake.md`, "Timed plan approval"): one or two lines per scene, the hero's journey, the length and the sound idea, with the deadline in the same message (10 minutes by default). Silence counts as approval: when the deadline passes, say in one line that you are starting on the reviewed plan. During the wait, set up the working folder and the shared `helpers.js`, and let the sound plan run — but don't start the art lanes, because that is the expensive work the plan governs.
3. **Plan the sound.** With the reviewed script, run the `sound-designer` agent for a cue sheet (beds and ambience per scene, a motif, effect cues on beats and seams, fades, deliberate silences, levels) and save it as `cues.md`. This runs beside phase 4, not before it, and never before Gate 1 has passed the script.
4. **Build the scenes.** Set up a working folder if the wait didn't: `cp -R "${CLAUDE_SKILL_DIR}"/toolkit/. .` (if that variable is not filled in, the `toolkit` folder sits next to this SKILL.md). Never build inside the plugin. Write the shared `helpers.js` first, then give each scene its own build lane — one agent, one plate file, its own range sheet as evidence (`node render.mjs probe_<n>.html --sheet-range <start>-<end> --fps 6 --dir qa/plate_<n>`, with `--crop x,y,w,h` for close-ups of small labels and textures) — following the contract in `references/build-lanes.md`. Use kit components (`references/components.md`) where they fit the subject and draw everything else yourself; copy a helper from `story_example.js` only when it genuinely fits. Build the scenes yourself, one after another, when the film is only one or two scenes, when they share a single continuous shot, or when the plates are too interdependent to draw apart.
5. **Assemble and render the QA set once.** Merge the plate files into `story.js` in script order, set `stages` and the plate order, then `python3 build.py story.js film.html`. Render the shared set once, into `qa/`: `node render.mjs film.html --sheet 1` (one frame per second), `--strips` (a 12-frame strip around every plate start), `--seams` (both sides of every transition and their overlay), then `node text_check.mjs film.html --json qa/text_check.json` and `node speed_check.mjs film.html`. The reviewers at Gate 2 reuse this `qa/` set and render only the extra frames they need, so nothing renders the same frames twice.
Gate 2. **Check and review what moves, not the stills.** Run `/doodle-art-animation:doodle-qa`, or by hand: the cheap checks in parallel (`text_check` for reading time and collisions, `speed_check` for seam and camera speed — `FAST` is advice, `SNAP` must be fixed — and `python3 smoke_test.py --work qa_smoke` for page errors, font failures, blank frames and silent audio), then the `film-reviewer`, `seam-reviewer` and, once a clip with sound exists, `audio-reviewer` agents in parallel on the shared `qa/` set. Open the strips and the seam sheets yourself and work through the QA checklist below; without the agents, apply the rubric in `references/film-grammar.md`. Then merge every fix list into one, apply it, and **re-run only the checks the change affects**: text edits need `text_check`, a seam or camera change needs `speed_check` and `seam-reviewer`, new art or a new beat needs its range sheet, `motion_check` and `film-reviewer`, a cue change needs `audio_check` and `audio-reviewer`. Don't re-run the whole gate for a one-line fix.
6. **Render**: `/doodle-art-animation:doodle-render`, or by hand `node render.mjs film.html film.mp4 --workers 6 --bitrate 3800k --strict-fonts` (a one-minute film lands near 20–30 MB), then `python3 motion_check.py film.mp4` against the targets and `python3 audio_check.py film.mp4 --starts <transition times>` (targets in `references/sound.md`). Leave out `--bitrate` only for a master you will re-encode: the grain makes constant-quality files huge (a 2.7-minute film was 1.36 GB). For films over two minutes, read "Long films" in `references/render.md`.
...
- **Ask, then review the plan.** Start with the intake round (`references/intake.md`), write the script to `script.md`, and pass Gate 1 (`script-reviewer`, then the timed plan card) before any art is drawn.
