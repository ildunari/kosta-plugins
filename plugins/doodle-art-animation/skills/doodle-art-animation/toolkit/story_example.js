...
/* One PLGA nanoparticle (NP·01), from the syringe to the slow release of its drug, in 4 plates plus title and end card.
   ...
   Seams (exit → entry, link, transition):
     0 → I    the drop lands in a pool of blood → a red blood cell beside NP·01 in a vein
              link: going into a surface (the blood) and out of a red object inside it → `through`, 1.8 s (one plunge)
     I → II   NP·01, tracked along the vein → the same particle, huge, in plasma
              link: scale (paper world → microscope world) → `lensIn`, 1.4 s, a snappy arrival
     II → III the coated particle, carried off to the right by the plasma → a dot moving right along a capillary
              ...
              lens-in) → whip `pan` to the right, 0.9 s, picking up the flow's speed; plate III's own camera then pulls back to reveal the tumour
     III → IV NP·01 lodged in the tumour, camera pulled back → the particle up close, swelling with water
              link: scale, motivated by a slow creep and a pulsing reticle; slower than seam I → II → `lensIn`, 2.1 s, heavy, with a long release (speed peaks at 40%)
     IV → V   the eroded particle → the small particle of the end card
              link: the same object becomes a symbol → `shape` morph, 1.6 s

   Rhythm: plunge 1.8 s · snap lens 1.4 s · whip 0.9 s · heavy lens 2.1 s · morph 1.6 s. Lengths and speed shapes both vary;
   nothing reads until a move lands (landAt(enter) + 0.3–0.5 s).
...
const P1 = {
  dur: 10, dark: false,
  enter: { type: 'through', dur: 1.8, momentum: false, from: { at: LAND, r: 30 }, to: (pl, t) => ({ at: rbcI(t), r: 32 }), fromFill: PAL.blood, toFill: PAL.blood },   // to r 32: inside the cell, so the colour field reads as its body
  ...
  overlay(t) {
    withAlpha(beat(t, 2.0, 8.0), () => stat(t - 2.0, { x: 610, y: 335, kicker: 'ONE MG OF 150 NM PLGA SPHERES', value: u => '≈ ' + countUp(435, u, 1.5) + ' billion', note: 'solid spheres, density ≈ 1.3 g/cm³' }));
...
const P2 = {
  dur: 10, dark: true, enter: { type: 'lensIn', dur: 1.4, dive: 1.8 },
...
const P3 = {
  dur: 12.8, dark: false, enter: { type: 'pan', dir: 'right', dur: 0.9 },   // the flow carries us on: the sheets keep sliding right, the way the particle was going
  ...
  cues: [[1.1, 'pop'], [1.45, 'scratch', { chars: 15 }], [1.8, 'chime', { f: 587 }], [4.4, 'plink'], [5.2, 'pop'], [5.55, 'scratch', { chars: 10 }], [7.2, 'chime', { f: 784 }], [8.3, 'scratch', { chars: 23, cps: 30 }]],
  draw(t) {
  ...
  overlay(t) {
    ...
    withAlpha(beat(t, 1.1, 7.3), () => stat(t - 1.1, { x: 720, y: 196, kicker: 'GAPS IN MANY TUMOUR VESSELS', value: u => typed('≈ 380–780 nm', u, 16), note: 'pore cut-off in most tumours tested (mice)', size: 56 }));
...
const P4 = {
  dur: 12, dark: true, enter: { type: 'lensIn', dur: 2.1, ease: E.shaped(0.4, 2, 4), land: 0.7 },   // longer than seam I → II, so it reads slower
  header: { num: 4, title: 'Slow Release', sub: 'water gets in, the drug gets out' },
  stage: { n: 4, name: 'RELEASE', prevN: 3 },
  log: t => { const day = lerp(0, 28, inv(2.0, 11.5, t));   // the same clock as the release chart
