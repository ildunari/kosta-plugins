...
**Minimum lengths.** lensIn and lensOut 1.4 s (dive ≤ 2×), zoom 1.9 s at `k` 4 (2.15 s at `k` 5), through 1.6 s, shape 1.3 s, iris 1.4 s, pan 0.8 s, wipe 0.8 s, bleed 1.5 s, burn 1.4 s, page 1.6 s, roll 1.3 s, erase 1.2 s, fade 1.0 s.
...
| `zoom` | One step on the scale ladder **within the same world** | Both plates pivot on the hero on an asymmetric ramp; the old scale grows (or shrinks) by `k` and fades while the new settles in; the HUD crossfades. A plate shrunk below full size is seen through a soft disc, so its world's edges never show. `dir`: 'in' (the default: the camera dives toward the hero, into finer detail) or 'out' (it pulls back to show more of the world; before v0.16.1 this was the default, so older stories with no `dir` zoomed out). `k` 3–5 (default 3.5; keep under 5% per drawing: k 4 needs 1.9 s, k 5 needs 2.15 s). | 1.9 s (1.9–2.4) | glide |
...
| `pan` | Same scale, **somewhere else along the journey** (downstream, next room) | Whip pan along one long sheet: both plates slide (`dir` 'left', 'right', 'up', 'down'), motion-blurred at speed, a faint fold shadow at the join, speed lines at full speed. The corner labels travel with the sheet. Both sheets are on screen at once, so a hero still in frame on the old sheet while the new one's hero slides in is seen twice (`speed_check` warns `TWICE`). Across a scale jump the second copy reads as a different object: use `zoom`, `lensIn`/`lensOut` or `through` for that, or let the old hero leave the frame before the new one enters. | 0.8 s (0.8–1.2) | whoosh |
...
| `erase` | A chalk or whiteboard film moving on to the next point, as a lecturer clears the board | A board eraser rubs across the old plate in overlapping up-and-down swipes, leaving a faint smear, and the new plate shows wherever it has been. Options: `dir` ('lr', the default, or 'rl'), `band` (eraser width, 190 px). Best on the chalk papers; on the notebook it reads as a gimmick. | 1.4 s | felt rubbing |
...
- **`ease`:** the easing for the whole transition, e.g. `ease: 'inOutSine'` (slow in and out), `'outBounce'` (lands with bounces), `'inBack'` (pulls back before going), `'outExpo'` (fast start, long settle). On presets with one main easing (lensIn, lensOut, zoom, pan, wipe, bleed, burn, page, roll, shape, hatch, erase, fade) it **replaces** that easing; on iris, through and custom transitions it reshapes the clock. Presets already shape their speed, so leave `ease` out unless you want a different feel (for example a heavier arrival: `ease: E.shaped(0.4, 2, 4)`, with `land: 0.7` so titles wait for it). A transition that feels rushed usually has its peak too early or its release too short: fix the shape (a longer release, a smaller ratio, blur) before adding length. Lengthening a symmetric ease only makes it uniformly slower.
