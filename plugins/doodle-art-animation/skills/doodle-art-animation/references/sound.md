...
The sound has several layers: pure harmonic stacks, filtered-noise beds, pitch sweeps at zooms, a scratch texture under every typed line, thumps on card landings, and real stereo. The engine renders all of this with an `OfflineAudioContext`, so the result is deterministic and exported as WAV.
