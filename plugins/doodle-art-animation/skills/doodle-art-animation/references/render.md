...
- **Narration:** a narrated film (one built with its `vo/voice.json`; `references/api.md`, "Narration") renders its voice with the rest of the track, and a full render writes the captions next to the MP4 (`film.mp4` → `film.srt`). The title line says `narrated`, and a clip the story asks for that voice.json lacks, or an unused one, prints a yellow `WARNING: narration`. Three modes of their own: `--srt [film.srt]` writes the captions alone; `--stems [--dir qa]` writes `stem_voice.wav`, `stem_rest.wav` and `speech.json` for `audio_check.py --narrated --stems qa` (`references/sound.md`, "Narration"); and `--animatic`, with any mode, draws placeholders instead of the plates' art, for judging the pacing with the voice before the art exists.
