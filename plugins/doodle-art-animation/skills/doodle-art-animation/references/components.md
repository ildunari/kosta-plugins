...
**Minimum size.** Text a viewer reads inside a component (`ai.agent` tool names, `ai.chat`, `ai.tokens`, `ai.attention`, `ai.network` labels, `tech.terminal`, `tech.code`, `earth.strata` layer names, `settle.map` place names, `lab.flask` and `life` sign labels, `KIT.caption`) is 22 px at `s: 1`, the `label` floor (`references/style.md`, "Text size and legibility"). So never scale those below `s: 1`, and never below the camera's pull-back: make them smaller with `w` (or fewer `lines`/`words`) instead. Text drawn as part of an object (browser chrome, server tags, paint-chip names and hex codes, stall signs, graduations, the chip's marking, token ids) is `decor` and may shrink with the drawing. Pure drawings (trees, planets, molecules, glassware) work down to about 0.4.
...
## KIT.settle: where people live (paper; all of it works at night)

Buildings, settlements, routes and traces. Things that stand on the ground (`house`, `hut`, `tent`, `tower`, `cart`, and a `ship` at its waterline) anchor at the **middle of their base**; `village`, `skyline`, `fields`, `market` and `ruins` anchor at the **left end of their ground line**; `map` anchors at its **top-left corner**; `bridge` at the **left end of its deck**; `road` draws along a `path` relative to `x`, `y`. On night plates (`dark: true`) fills darken and `lit` (default: `dark`) turns on windows, lamps and hearths, each with its own flicker.

| Call | Own options | What it looks like and how it moves | Good for |
|---|---|---|---|
| `settle.house(t, o)` | `style, w, smoke, lit, ground` | A house drawn up wall, roof, then windows and door. `style: 'cottage'` (tiled roof, plastered wall), `'townhouse'` (three brick storeys, slate gable, attic window) or `'farmhouse'` (barn-roofed, board walls, X-braced door, a weather vane that turns). Chimney smoke drifts; lit windows flicker. | Homes, streets, "where people lived", any settled scene |
| `settle.hut(t, o)` | `kind, w, smoke, lit, ground` | `kind: 'round'` is a mud-walled roundhouse under a conical thatch; `'long'` is a low longhouse under a deep thatch. The thatch fringe sways, smoke seeps from the apex, and a fire glows in the doorway when lit. | Early settlement, villages, vernacular building, archaeology |
| `settle.tent(t, o)` | `kind, w, color, smoke, lit, ground` | `kind: 'camp'` is an A-frame with guy ropes, a pennant and a door flap that swings; `'yurt'` is a felt yurt with a lattice wall, a painted door whose curtain sways and a smoking crown. | Nomads, expeditions, camps, migration, fieldwork |
| `settle.tower(t, o)` | `kind, w, h, lit, flag, beam, ground` | Stone or timber that builds up from the ground. `kind: 'castle'` (crenellated keep, arrow slits, a rippling flag), `'lighthouse'` (striped tower on rocks; the beam sweeps round and flashes when it faces you), `'watchtower'` (braced timber legs, ladder, cabin with a lookout). | Defence, navigation, borders, landmarks, "keeping watch" |
| `settle.village(t, o)` | `w, h, n, smoke, lit, people` | A seeded cluster of `n` houses in two rows on a meadow, with trees, a winding main path, footpaths to the back doors and people walking it. Houses build one after another, chimneys smoke, windows glow at night. | Settlement, community, rural history, "a place on the map" |
| `settle.skyline(t, o)` | `w, h, era, lit, haze` | A seeded city silhouette with a paler row behind. `era: 'modern'` gives towers, setbacks, antennas with blinking lights and a crane whose jib swings; `'old'` gives gables, domes, steeples, flagged towers and birds wheeling. Haze drifts behind; by day light slides across the glass, at night about half the windows are lit and switch on and off. | Cities, growth, economics, industry, "the modern world" |
| `settle.road(t, o)` | `path, kind, width, traffic` | A route that draws on from its first point, seen from above. `kind: 'road'` (paved, dashed centre line, cars both ways, headlights at night), `'path'` (a dirt track where footprints appear behind a walker) or `'rail'` (ballast, sleepers, rails and a small train with smoke). | Trade, journeys, infrastructure, commuting, supply chains |
| `settle.bridge(t, o)` | `kind, w, h, n, traffic` | A crossing over a river gorge: banks and flowing water first, then the structure. `kind: 'arch'` (`n` stone arches with voussoirs and rippling reflections), `'suspension'` (towers, hung cables, cars crossing) or `'plank'` (a rope bridge that sags and dips under a walker). | Connection, engineering, obstacles, "getting across" |
| `settle.ship(t, o)` | `kind, w, dir, sea, wake, lit` | A vessel that bobs and rolls with a trailing wake. `kind: 'sail'` (two masts, billowing square sails, jib, rigging, pennant), `'boat'` (a rower pulling the oars) or `'steam'` (cabins, a smoking funnel, portholes lit at night). `dir: -1` sails left; `sea: false` leaves out the strip of water so you can put it on your own sea. | Trade, exploration, migration, coasts, islands |
| `settle.cart(t, o)` | `w, load, wheels, spin, dust` | A wooden cart with shafts, its wheels turning at `spin` (0 parks it), the bed jolting and dust puffing behind. `load: 'hay'` (straws sway), `'sacks'`, `'barrels'` or `'none'`; `wheels: 2` makes it a wagon. Move it by animating `x`. | Haulage, harvest, markets, pre-industrial transport |
| `settle.fields(t, o)` | `w, h, cols, rows, hedges` | A patchwork of farm fields receding up from the front edge: wheat, green crop, ploughed furrows and fallow ground, with hedgerows, bushes and a few trees. Crop rows sway and gusts run across the wheat. Patches fill in from the front corner. | Agriculture, land use, food, the countryside, enclosure |
| `settle.market(t, o)` | `w, n, goods, people, lit` | `n` stalls on a cobbled street: striped awnings with a fluttering fringe, swinging signs, sellers behind counters of `goods` (`'fruit'`, `'jars'`, `'cloth'`, `'bread'`) and people walking past. Lanterns glow at night. | Trade, prices, cities, everyday life, exchange |
| `settle.map(t, o)` | `w, h, title, route, labels, compass` | A hand-drawn map card: a seeded coastline with contour rings, hills, woods and an island, drifting wave marks, a dashed `route` (points in 0..1 of the card) with a little boat travelling it, a compass whose needle settles, a scale bar, `labels` (`[[text, u, v], …]`) and a title cartouche. Stays paper-coloured on night plates. | Geography, voyages, empires, "here is where we are going" |
| `settle.ruins(t, o)` | `w, h, columns, vines` | What is left of a building: a broken wall with an arch, fluted columns (some whole under a lintel, some snapped), fallen drums and rubble. Stones build up from the ground, vines climb and sway, dust drifts in the light. | Collapse, antiquity, archaeology, time passing, empires ending |

Notes:

- The pieces are designed to sit together: `village` or `skyline` on the horizon, a `road` or `bridge` leading into it, `fields` in front, a `market` or `cart` in the foreground, a `ship` on the water, `ruins` for the same place centuries later. Use two or three, not all of them.
- `village` draws whole houses inside itself, so it is the most expensive component here; at `h` under about 150 px its houses stop reading and a couple of `house` calls look better.
- Keep the text-bearing ones legible: `market` signs and `map` labels blur below about `s: 0.75`. Scale a map down with `w`/`h` instead.
- `ship` with `sea: true` draws its own patch of water; over `earth.sea` or your own water, pass `sea: false`.
- `road` caches its band geometry per `path`, so pass a fixed `path` and animate `x`, `y` or `draw` instead of rebuilding the points each frame.
...
The shared helpers in `kits/_kit.js` are `KIT.memo`, `KIT.opts`, `KIT.at`, `KIT.lw` (line width under scale), `KIT.ph`/`KIT.pop` (draw-on phases), `KIT.cyc` (repeating cycles), `KIT.rrect`, `KIT.ribbon`, `KIT.offset`, `KIT.cubic`, `KIT.rot`, `KIT.shadow`, `KIT.caption` (22 px `label` by default; `screen: true` keeps its size under the camera, `backing: true` puts it on a paper halo, `role: 'decor'` for FIG. numbers), `KIT.inkOf`, `KIT.mutedOf` (lines) and `KIT.labelOf` (secondary text).
